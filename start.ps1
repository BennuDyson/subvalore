...
param (
    [switch]$SkipInstall,    # skip pip install (faster restarts)
...
function Fail       { param($msg) Write-Host "`n!! $msg" -ForegroundColor Red; exit 1 }
...
if (-not (Test-Path "$Root\.env")) {
    ...
    Write-OK ".env created from .env.example (using default credentials)"
} else {
...
Write-Host "   Waiting for PostgreSQL to be healthy..." -ForegroundColor DarkGray
$maxWait = 60
$elapsed = 0
do {
    Start-Sleep -Seconds 2
    $elapsed += 2
    $status = docker inspect subvalore_postgres --format "{{.State.Health.Status}}" 2>$null
...
if ($SkipInstall) {
    Write-Warn "Skipping pip install (-SkipInstall flag set)"
} else {
...
# Generate initial migration file if versions/ is empty
...
& $venvPath\Scripts\uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
